...
export function aiErrorText(e: unknown): string {
  if (e instanceof AiError) {
    switch (e.code) {
      case 'no_account':
        return 'Включите синхронизацию в Настройках — она нужна для авторизации.';
      case 'unauthorized':
        return 'Сервер не признал аккаунт. Проверьте синхронизацию.';
      case 'forbidden':
        return 'Аккаунт не в списке разрешённых на сервере.';
      case 'rate_limit':
        return 'Слишком часто. Подождите немного.';
      case 'provider':
        return `Провайдер отказал: ${e.message}`;
      case 'network':
