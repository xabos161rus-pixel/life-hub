...
import { Sun, ListTodo, NotebookText, Users, LayoutGrid } from 'lucide-react';
...
const tabs = [
  ...
  { to: '/more/family', label: 'Семья', icon: Users, end: false },
  // end=true: «Ещё» подсвечивается только на самом списке /more, а не на его
  // подстраницах (иначе на /more/family горели бы сразу два таба).
  { to: '/more', label: 'Ещё', icon: LayoutGrid, end: true },
];
    ...
    <nav className="z-30 shrink-0 border-t border-hairline bg-elevated pb-[clamp(6px,env(safe-area-inset-bottom),8px)]">
      <div className="mx-auto flex max-w-lg px-1">
        {tabs.map(({ to, label, icon: Icon, end }) => {
          const showBadge =
            (label === 'Ещё' && backupStale) || (label === 'Семья' && familyUnread);
          return (
            <NavLink
              key={to}
              to={to}
