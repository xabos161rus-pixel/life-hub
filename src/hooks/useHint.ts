...
/** Убрать подсказку навсегда БЕЗ нажатия крестика — потому что человек
 *  сделал ровно то, чему она учила.
 *
 *  Подсказка про быстрый ввод занимает над списком 165px и висит там, пока её
 *  не закроют руками: тот, кто уже понял, платит этой полосой каждый заход.
 *  Совет, которому последовали, свою работу сделал. */
export async function dismissHintOnUse(id: string): Promise<void> {
