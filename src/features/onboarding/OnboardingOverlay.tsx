...
const SLIDES: { icon: LucideIcon; title: string; text: string }[] = [
  ...
  },
  {
    ...
    text: isTouch
      ? 'Быстрый ввод одной строкой: «завтра в 10 позвонить маме» — дата и время распознаются сами. Свайп по задаче вправо — выполнить, влево — перенести или удалить, удержание — перетащить в другой проект.'
      : 'Быстрый ввод одной строкой: «завтра в 10 позвонить маме» — дата и время распознаются сами, Enter добавляет задачу. Потяните задачу мышью вправо — выполнить, влево — перенести или удалить.',
  },
  {
    icon: NotebookText,
    title: 'Заметки',
  ...
  },
  {
    icon: LayoutGrid,
    title: 'Раздел «Ещё»',
    text: 'Семейный чат и звонки, финансы, фокус-таймер, обучение, места и энергия. Здесь же — настройки, автокопии и синхронизация. А ссылками и текстом из других приложений можно делиться прямо в Life Hub — они сразу станут задачей или заметкой.',
  },
  {
    icon: ShieldCheck,
    title: 'Защитите свои данные',
    text: 'Включите синхронизацию — задачи, заметки и цели будут на всех ваших устройствах, а зашифрованная копия сохранится в облаке под вашим ключом и переживёт потерю телефона. А уведомления доставят напоминания даже при закрытом приложении. Всё это — в «Ещё → Настройки», и приложение мягко напомнит об этом на «Сегодня».',
  },
  {
    icon: Lightbulb,
