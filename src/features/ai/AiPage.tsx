          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pb-3">
            {!messages.length && (
              <EmptyState
                ...
                hint="Пока отвечает заглушка — провайдер подключается на следующем шаге."
              />
            )}
            {messages.map((m) =>
              m.role === 'user' ? (
    ...
    <div className="grid grid-cols-[1.25rem_1fr] gap-x-1">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1.5 flex items-center gap-3 font-mono text-[0.7rem] text-muted">
          ...
          <button aria-label="Повторить" className="p-1 active:opacity-60 disabled:opacity-30" disabled={busy} onClick={onRetry}>
            <RotateCcw size={13} />
          </button>
        </div>
      </div>
...
function Thinking() {
  return (
    <div className="grid grid-cols-[1.25rem_1fr] gap-x-1">
      ...
      <p className="font-mono text-xs text-muted">думает…</p>
    </div>
  );
}
